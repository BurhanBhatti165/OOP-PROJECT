...
Z::Z()
{
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			temp = i * 4 + j;
			if (temp == 4 || temp == 5 || temp == 9 || temp == 10)
				SHAPEarr[i][j] = 7;
			else
				SHAPEarr[i][j] = 0;
		}
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			temp = i * 4 + j;
			if (temp == 4 || temp == 5 || temp == 9 || temp == 10)				/*for 0 degree shape*/
				Degree0[i][j] = 7;
			else
				Degree0[i][j] = 0;
			if (temp == 2 || temp == 6 || temp == 5 || temp == 9)				/*for 90 degree shape*/
				Degree90[i][j] = 7;
			else
				Degree90[i][j] = 0;
			if (temp == 5 || temp == 6 || temp == 10 || temp == 11)				/*for 180 degree shape*/
				Degree180[i][j] = 7;
			else
				Degree180[i][j] = 0;
			if (temp == 6 || temp == 10 || temp == 9 || temp == 13)				/*for 270 degree shape*/
				Degree270[i][j] = 7;
			else
				Degree270[i][j] = 0;
		}
...
void Z::defaultPosi(RenderWindow& window)
{
	PosiX = 0, PosiY = 3;
...
void Z::draw(RenderWindow& window, int PosiX, int PosiY)
{
...
void Z::moveLeft(RenderWindow& window)
{
	if (Compare(Degree270) && PosiX >= 16) {
		draw(window, 16, PosiY);
	}
	else if (!Compare(Degree270) && PosiX >= 17) {
...
void Z::moveRight(RenderWindow& window)
{
	if (Compare(Degree270) && PosiX >= 16) {
		draw(window, 16, PosiY);
	}
...
void Z::moveDown(RenderWindow& window)
{
...
void Z::rotate(RenderWindow& window)
{
	if (Compare(Degree270) && PosiX >= 16) {
		draw(window, 16, PosiY);
	}
		...
		if (Compare(Degree0) && PosiY <= 0)
		{
			draw(window, PosiX, 0);
			PosiY = 0;
		}
		else if (!Compare(Degree0) && PosiY <= -1)
		{
			draw(window, PosiX, -1);
			PosiY = -1;
		}
		else if (Compare(Degree180) && PosiY >= 6)
		{
			draw(window, PosiX, 6);
			PosiY = 6;
		}
		else if (!Compare(Degree180) && PosiY >= 7) {
			draw(window, PosiX, 7);
			PosiY = 7;
		}
		else {
